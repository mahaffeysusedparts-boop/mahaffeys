...
import { Activity, Download, FileText, Printer, TrendingUp, Scale } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const { tickets, shipments, vehicles } = useMemo(
    () => ({
      tickets: storageService.getTickets().filter((ticket) => ticket.status === 'COMPLETED'),
      shipments: storageService.getShipments(),
      vehicles: storageService.getPullYardVehicles(),
    }),
    [],
  );

  const since = Date.now() - days * 86400000;
  const active = tickets.filter((ticket) => new Date(ticket.createdAt).getTime() >= since);
  const settled = shipments.filter((shipment) => shipment.settlement && new Date(shipment.settlement.settledAt).getTime() >= since);
  const metrics = deriveOperationsMetrics({ tickets: active, shipments: settled, vehicles, openQueueCount: 0, delayedTaskCount: 0 });

  const trend = Array.from({ length: Math.min(days, 30) }, (_, index) => {
    const date = new Date(Date.now() - (Math.min(days, 30) - 1 - index) * 86400000).toDateString();
    return {
      day: index === Math.min(days, 30) - 1 ? 'Today' : new Date(date).toLocaleDateString(undefined, { month: 'numeric', day: 'numeric' }),
  ...
  // ── Scale traffic — audit-stamped readings per platform + journal-fed daily LBS ──
  ...
  const scaleTraffic = useMemo(() => {
    const events = storageService.getScaleEvents().filter((event) => new Date(event.detectedAt).getTime() >= since);
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
        <section className="flex flex-col sm:flex-row justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400">Shared transaction metrics for management reporting.</p>
          </div>
          <div className="flex gap-2">
            <Link to="/operations">
              <Button className="bg-emerald-600 hover:bg-emerald-500"><Activity className="w-4 h-4 mr-2" />Operations</Button>
            </Link>
            <select value={range} onChange={(event) => setRange(event.target.value)} className="rounded-xl bg-slate-900 border border-slate-700 px-3 text-sm">
              <option value="1">Today</option>
              <option value="7">7 days</option>
              <option value="30">30 days</option>
              <option value="365">YTD</option>
            </select>
            <Button onClick={() => setCustomReportOpen(true)} className="bg-emerald-600 hover:bg-emerald-500">
              <FileText className="w-4 h-4 mr-2" />Custom Report
            </Button>
            <Button onClick={() => window.print()} variant="outline" className="border-slate-700"><Printer className="w-4 h-4" /></Button>
          </div>
        </section>

        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
          {[
            ['Tickets', metrics.completedTickets],
            ['Payouts', money(metrics.payouts)],
            ['Revenue', money(metrics.shipmentRevenue)],
            ['Gross margin', money(metrics.grossMargin)],
            ['Lbs in', active.reduce((total, ticket) => total + ticketWeight(ticket), 0).toLocaleString()],
            ['Vehicles', metrics.vehiclesProcessed],
          ].map(([label, value]) => (
            <Card key={String(label)} className="bg-slate-900 border-slate-800 rounded-2xl">
              <CardContent className="p-4">
                <p className="text-xs text-slate-400">{label}</p>
                <p className="font-mono font-black text-lg mt-2">{value}</p>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="grid lg:grid-cols-2 gap-5 mt-5">
            <Card className="bg-slate-900 border-slate-800 rounded-2xl">
              <CardHeader><CardTitle>Daily payout vs revenue</CardTitle></CardHeader>
            ...
            <Card className="bg-slate-900 border-slate-800 rounded-2xl">
              <CardHeader><CardTitle>Ticket volume by type</CardTitle></CardHeader>
              <CardContent className="h-72">
                <ResponsiveContainer>
          ...
          <TabsContent value="materials" className="mt-5">
            <Card className="bg-slate-900 border-slate-800 rounded-2xl">
              <CardHeader className="flex-row justify-between">
                <CardTitle>Payout mix by material</CardTitle>
                <Button onClick={() => downloadCsv('material-summary.csv', cats.map((item) => ({ Material: item.name, Payout: item.value.toFixed(2) })))} className="bg-emerald-600">
          ...
          <TabsContent value="scales" className="grid lg:grid-cols-2 gap-5 mt-5">
            <Card className="bg-slate-900 border-slate-800 rounded-2xl">
              <CardHeader className="flex-row justify-between">
                <CardTitle className="flex items-center gap-2"><Scale className="w-4 h-4 text-emerald-400" />Readings by platform</CardTitle>
                <Button
            ...
            <Card className="bg-slate-900 border-slate-800 rounded-2xl">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Scale className="w-4 h-4 text-amber-400" />LBS crossed the platform</CardTitle>
