...
export const operationsService = {
  getSnapshot(): OperationsSnapshot {
    ...
    const fleetTools = storageService.getTools();
    const equipmentDue = getOverdueEquipment(ruleFor(rules, 'EQUIPMENT_SERVICE_DUE')?.threshold ?? 7);
    const toolOverdue = getOverdueCheckouts();
    const fleetEntries = [
      ...equipmentDue.map((info) => queueEntry({ id: `equipment-${info.item.id}`, type: 'FLEET', title: info.item.name, detail: info.isOverdue ? `Service overdue by ${info.daysUntilDue ?? 0}d` : `Service due in ${info.daysUntilDue}d`, status: info.item.status || 'Active', priority: info.isOverdue ? 'URGENT' : 'ATTENTION', createdAt: info.item.nextServiceDue || info.item.lastServiceDate || new Date().toISOString(), link: '/fleet' })),
      ...toolOverdue.map((checkout) => queueEntry({ id: `tool-checkout-${checkout.id}`, type: 'FLEET', title: fleetTools.find((tool) => tool.id === checkout.toolId)?.name || 'Tool', detail: `Checked out by ${checkout.checkedOutBy} · past due back`, status: 'OUT', priority: 'ATTENTION', owner: checkout.checkedOutBy, createdAt: checkout.dueBackAt || checkout.checkedOutAt, link: '/fleet' })),
    ];
    const queues: Record<OperationsQueueType, OperationsQueueEntry[]> = { INTAKE: intake, VEHICLE: vehicles.filter((vehicle) => vehicle.status !== 'CRUSHED').map(vehicleQueue), SHIPMENT: shipmentEntries, CONTAINER: containerEntries, COMPLIANCE: compliance, FLEET: fleetEntries };
    ...
    equipmentDue.forEach((info) => add('EQUIPMENT_SERVICE_DUE', true, `equipment-service-${info.item.id}`, 'FLEET', info.isOverdue ? 'URGENT' : 'ATTENTION', `${info.item.name} service ${info.isOverdue ? 'overdue' : 'due soon'}`, info.isOverdue ? `Service is ${info.daysUntilDue ?? 0} day(s) past due.` : `Service is due in ${info.daysUntilDue} day(s).`, 'Log the maintenance service and set the next due date.', '/fleet'));
    toolOverdue.forEach((checkout) => add('TOOL_CHECKOUT_OVERDUE', true, `tool-overdue-${checkout.id}`, 'FLEET', 'ATTENTION', `${fleetTools.find((tool) => tool.id === checkout.toolId)?.name || 'Tool'} is overdue for return`, `Checked out by ${checkout.checkedOutBy} and past the due-back date.`, 'Recover the tool and check it back in on the Fleet & Tools page.', '/fleet'));
