...
export async function analyzeVisionScan(scan: { id: string; purpose: VisionPurpose; storage_path: string }) {
  ...
  try {
    ...
    const response = await fetch(`${workerUrl.replace(/\/$/, "")}/analyze`, { method: "POST", body: form, signal: AbortSignal.timeout(30_000) });
    if (!response.ok) throw new Error("worker response was not successful");
    worker = await response.json() as WorkerResult;
  } catch {
    await query("UPDATE vision_scans SET status = 'failed', error_message = 'Vision worker unavailable. Retry when the local worker is online.' WHERE id = $1", [scan.id]);
    return;
  }
